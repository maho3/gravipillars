...
while strcmp(init{1},'go')
    ...
    switch oSC{1}.path
        ...
        case 'particleField'
            toggParticleField=holdVar{1,1};
        case 'deflect'
            toggParticleDeflect=holdVar{1,1};
        case 'repulse'
            toggParticleRepulse=holdVar{1,1};
        case 'magO'
            magO=holdVar{1,1};
        case 'magP'
            magP=holdVar{1,1};
    ...
    for n=1:N                       %math and physics to calculate position
        ...
        if toggParticleRepulse==1;
            for rB=1:N
                if rB~=n
                    rPosVec=[x{rB}(k)-x{n}(k),y{rB}(k)-y{n}(k)];
        ...
        if toggFrictionField==1     %calculate and add friction accel
            if d>=frictionRadius
                velMag=norm([vx{n}(k),vy{n}(k)]);
                velDirect=[vx{n}(k),vy{n}(k)]/velMag;
        ...
        if toggParticleDeflect==1
            for mm=1:(n-1)
                if (k-prevDeflect{n}(mm))>=dBuffer  %ensure particles don't trap themselves in alternating velocity
                    dPosVecNM=[x{n}(k)-x{mm}(k),y{n}(k)-y{mm}(k)];
                    ...
                    if magDPosVec<=pDRad            %if particles are within a each other's radii
                        vn=[vx{n}(k+1),vy{n}(k+1)];  %determine velocity vectors
                        vm=[vx{mm}(k+1),vy{mm}(k+1)];
    ...
    for n=1:N                       %plot
        pos{n}=struct('path',sprintf('/cat%d',n),'tt','fff','data',{{x{n}(k),y{n}(k)}});
    end
